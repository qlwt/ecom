...
const paginator_new = function(store: asc.AtomStore, search: Paginator_Search) {
    return gs.paginator_new_list<gs.Rem_Node<"tmplit">, Paginator_Search, string | null, number>({
        init: {
            data: [],
                            ...
                            success: data => {
                                api.data.input([
                                    ...
                                    ...(data.slice.tmplit?.nodes ?? []).map(n => {
                                        return store.reg(rem.tmplit.register).reg({ id: n.id })
                                    })
                                ])
...
export type EPSelectList__TmplIt_Props = {
    readonly include_hidden: 0 | 1
    readonly include_private: 0 | 1
...
export const EPSelectList_TmplIt: r.FC<EPSelectList__TmplIt_Props> = props => {
                ...
                sc.osignal_new_pipeflat(
                    sc.osignal_new_listpipe(paginator.data, ndata => {
                        return sc.osignal_new_memo(
                            reg(rem.tmplit.joins.core())({ id: ndata.statics.id })
                        )
                    }),
                    nodes => {
...
export default EPSelectList_TmplIt
