...
const paginator_new = function(store: asc.AtomStore, search: Paginator_Search) {
    return gs.paginator_new_list<gs.Rem_Node<"tmplpr">, Paginator_Search, string | null, number>({
        init: {
            data: [],
                            ...
                            success: data => {
                                api.data.input([
                                    ...
                                    ...(data.slice.tmplpr?.nodes ?? []).map(n => {
                                        return store.reg(rem.tmplpr.register).reg({ id: n.id })
                                    })
                                ])
...
type EL__Card_Props = {
    readonly node: gs.Rem_JoinData<"tmplpr">
...
export type EPSelectList__TmplPr_Props = {
    readonly include_hidden: 0 | 1
    readonly include_private: 0 | 1
...
export const EPSelectList_TmplPr: r.FC<EPSelectList__TmplPr_Props> = props => {
                ...
                sc.osignal_new_pipeflat(
                    sc.osignal_new_listpipe(paginator.data, ndata => {
                        return sc.osignal_new_memo(
                            reg(rem.tmplpr.joins.core())({ id: ndata.statics.id })
                        )
                    }),
                    nodes => {
...
export default EPSelectList_TmplPr
