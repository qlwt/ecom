...
export type ELCon__CardTmplIt_Mtops = Readonly<{
    readonly node: cc.RemDef["tmplit"]["joins"]["core"]
}>

export const ELCon_CardTmplIt: r.FC<ELCon__CardTmplIt_Mtops> = props => {
    ...
    const href = urlmap.console.edit_tmplit({ id: props.node.id })
    ...
    return <EPCardImg_View>
        <EPCardImg_LayerBActions justify={`start`}>
            <EPAction_BtnClick
                ...
                event_click={() => {
                    dispatch(rem.tmplit.act.delete({
                        body: {
                            ids: [props.node.id]
                        },
                    }))
