...
import type { Rem_Index, Rem_JoinFactory, Rem_NodeDef, Rem_Result } from "@src/rem/type/result"
...
export type NRem_JoinNew_Params<TName extends keyof cc.RemDef, Variant extends keyof cc.RemDef[TName]["joins"]> = {
    readonly table_name: TName
    readonly self_variant: Variant
    readonly rem_new: () => Rem_Result
}

export const nrem_join_new = function <TName extends keyof cc.RemDef, Variant extends keyof cc.RemDef[TName]["joins"]>(
    params: NRem_JoinNew_Params<TName, Variant>
): Rem_JoinFactory<TName, Variant> {
    return () => ({ reg }) => {
        const rem = params.rem_new()

        return reg(asc.atomremnode_join_root<Rem_Index, Rem_NodeDef<"item">, {}>({
            link: () => index => reg(rem[params.table_name as "item"].register).reg(index),
                            ...
                            if (join.status_optional) {
                                return atomremnode_join_step(
                                    atomremnode_join_prop_optional({
                                        ...
                                        transformer: (param: asc.AtomRemNode__Data<Rem_NodeDef<"item">>) => {
                        ...
                        case "backwards":
                            ...
                            return atomremnode_join_sort(
                                asc.atomremnode_join_array_flat({
                                    transformer: (param: asc.AtomRemNode__Data<Rem_NodeDef<"item">>) => reg(idx).reg({
                                        deleted: 0,
                                        [join.target_field as "item__id"]: param.data![join.self_field as "id"] as string,
                                    }),
