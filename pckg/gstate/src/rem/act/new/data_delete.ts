...
import type { Rem_DataActDelete_Config, Rem_DataActDelete_RParams, Rem_Node, Rem_NodeDef, Rem_Result } from "@src/rem/type/result"
import { remx_auth__state } from "@src/remx/auth/internal/state"
import { promise_new_remdeps } from "@src/util/promise/remdeps"

export type NRemAct_NewDataDelete_Params<TName extends keyof cc.RemDefData> = {
    readonly table_name: TName

    readonly config: Rem_DataActDelete_Config
    readonly rparams: Rem_DataActDelete_RParams<TName>

    readonly rem_config: Rem_Config
    readonly rem_new: () => Rem_Result
}

export const nrem_act_new_data_delete = function <TName extends keyof cc.RemDefData>(
    params: NRemAct_NewDataDelete_Params<TName>
): asc.AtomAction {
    return ({ reg }) => {
        ...
        const action = function() {
            ...
            const promise = promise_new_remdeps({
                ...
                request_new: l_params => capi.send_rest_data_delete(
                    params.table_name,
                    {
                        ...params.rparams,
            ...
            for (const id of params.rparams.body.ids) {
                reg(asc.atomremnode_action_patch_set<Rem_NodeDef<TName>>({
                    ...
                    node: () => {
                        return reg(
                            params.rem_new()[params.table_name as "item"].register
