...
import type { Rem_LoaderGet_Index, Rem_Result } from "@src/rem/type/result"
...
export type NRem_LoaderNewGet_Params<TName extends keyof cc.RestData> = {
    readonly table_name: TName
    readonly config: Rem_Config
    readonly rem_new: () => Rem_Result
}

export const nrem_loader_new_get = function <TName extends keyof cc.RestData>(
    params: NRem_LoaderNewGet_Params<TName>
) {
    return asc.atomfamily_new({
        key: (index: Rem_LoaderGet_Index<TName>) => stringify_stable(index),

        get: (query: Rem_LoaderGet_Index<TName>) => {
